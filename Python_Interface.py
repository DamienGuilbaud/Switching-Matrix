...
for i in range (len(path)-1):
    ...
    if (edge_data["edge_type"] == "x_switch"):
        ...
        switch = [x,y,3,0]
        switch_list.append(switch)
        #only difference between nodes is x and y coord, calc coord for node by using largest x coord and smallest y coord
...
# FOR BATCH BASED!!!!!!!
# switch_list_len = len(switch_list)
...
for i in range(len(switch_list)):
    ...
    # time.sleep(dy)
    arduino.write(bytes("{},{},{},{}\n".format(switch_list[i][0], switch_list[i][1], switch_list[i][2], switch_list[i][3]), 'utf-8'))
    time.sleep(dy)
