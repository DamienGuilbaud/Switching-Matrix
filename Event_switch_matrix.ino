...
void printout() {
  byte * stored = io_exp_1.returnValue();
...
void change_switch(int x, int y, int s, int c) {
      ...
      //io_exp_1.writePin(port_num, port_pin,c); // set a single pin on port 0 (pin 5 numerically on the chip)
      exp_list[y].writePin(port_num, port_pin,c);
      //Serial.println(exp_list[0]);
